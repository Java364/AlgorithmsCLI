...
public class WaysToSumUsingArrayCLI extends AbstractCLI implements CommandLineInterface{
	...
	public void run() {
		...
		int result = new WaysToSumUsingArray().waysToSumToNUsingArrayElementsWithRepetitionExecute(parsedSequence, sum);	
		System.out.format("To represent %d we can such ways: %d\n", sum, result);
	}
	
	@Override
	protected boolean promptSequence() {
        System.out.print("Please enter sequence (e.g '1 2 3') of possitive integer numbers: ");
        ...
        return isPossitiveElement(this.parsedSequence);
    }
	
	private boolean isPossitiveElement(int[] sequence) {
